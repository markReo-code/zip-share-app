...
仕事で一時的なファイル共有サービスを利用する機会が多く、もっとシンプルな操作感で共有できるアプリを自分でも作ってみたいと考えたのがきっかけです。  
複数ファイルのアップロード、ZIP 化、期限付きダウンロードリンクの発行に機能を絞り、**手軽に使えるファイル共有体験**を目指しました。

実装では、以前から触ってみたかった **Cloudflare Workers / R2 / D1 と Hono** を採用しました。  
ファイル本体を R2、メタ情報と有効期限を D1 に分けて保存し、Cloudflare 上でのサーバーレス構成として実装しています。

## 技術スタック

- フロントエンド：TypeScript / React 19 / Next.js 15.5.18（App Router）/ Tailwind CSS v4 / Heroicons
